...
async function openAdminPage(
    ...
    const main = page.getByRole("main", { name: mainName });
...
async function scheduleOffboarding(
    ...
    await test.step(`Schedule ${action.toLowerCase()} offboarding`, async () => {
        ...
        await expect(
            dialog.getByRole("radio", { name: "Deactivate" }),
            "Deactivate is selected by default",
        ).toBeChecked();
        await expect(
            dialog.getByRole("checkbox", { name: "Revoke sessions" }),
            "Session revocation is enabled by default",
        ).toBeChecked();
        await expect(
            dialog.getByRole("checkbox", { name: "Revoke tokens" }),
            "Token revocation is enabled by default",
        ).toBeChecked();

        await form.setRadio("Action", action, dialog);
        await form.fill(dialog.getByLabel("Scheduled for"), dateTimeLocal(scheduledAt));
        await form.setInputCheck("Revoke sessions", revokeSessions, dialog);
        await form.setInputCheck("Revoke tokens", revokeTokens, dialog);
        ...
        await expect(
            page.getByRole("button", { name: "Cancel Offboarding" }),
...
async function runDueOffboardings(context: BrowserContext) {
    ...
    await test.step("Run the due offboarding schedule", async () => {
        const systemTasks = await openAdminPage(
            context,
            "Dashboards",
            "System Tasks",
            "System Tasks",
...
test.describe("User offboarding", () => {
    ...
    test("Schedules, lists, cancels, and reschedules an offboarding", async ({
        ...
        await test.step("Cancel from the user details page", async () => {
            await pointer.click("Cancel Offboarding", "button");

            const dialog = page.getByRole("dialog", { name: "Cancel scheduled offboarding" });
            await expect(dialog, "Cancel offboarding dialog opens").toBeVisible();
            await expect(dialog, "Cancel dialog shows the selected action").toContainText(
                "Action: Delete",
            );
            await expect(
                dialog,
            ...
            await expect(dialog, "Cancel dialog shows token revocation is disabled").toContainText(
                "Revoke tokens: No",
            );

            await dialog.getByRole("button", { name: "Cancel offboarding" }).click();
    ...
    test("Rejects an offboarding scheduled in the past", async ({
        ...
        const scheduledInput = dialog.getByLabel("Scheduled for");
    ...
    test("Bulk-cancels pending offboardings", async ({ page, form, pointer }, testInfo) => {
        ...
        const dialog = page
            .getByRole("dialog")
            .filter({ has: page.getByRole("button", { name: "Cancel Offboardings" }) });
        await expect(dialog, "Bulk cancellation dialog opens").toBeVisible();
        await expect(dialog, "Bulk cancellation includes both users").toContainText(
            users[0].username,
        );
        await expect(dialog, "Bulk cancellation includes both users").toContainText(
            users[1].username,
        ...
        await dialog.getByRole("button", { name: "Cancel Offboardings" }).click();
    ...
    test("Executes a scheduled user deactivation", async ({
        ...
        const activeTerm = page.locator("ak-user-info-card").getByRole("term", { name: "Active" });
        await expect(
            activeTerm.locator("..").getByRole("status", { name: "No" }),
        ...
        await expect(completedRow, "Completed deactivation appears in history").toContainText(
            "Completed",
        );
